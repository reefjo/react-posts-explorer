  ...
  */
  ...
  useEffect(() => {
    ...
    const allowedPosts = posts.filter((post) => allowedAuthorIds.includes(post.userId) )
    console.log("posts:",posts,"allowedPosts:", allowedPosts);
    const totalPages = Math.max(1, Math.ceil(allowedPosts.length / POSTS_PER_PAGE))

    const nextPageStartIndex = (currentPage - 1) * POSTS_PER_PAGE
    const currentPosts = allowedPosts.slice(nextPageStartIndex, nextPageStartIndex + POSTS_PER_PAGE);

    const toggleAuthorFilter = (id) => {
    var newAllowedIds = [...allowedAuthorIds];
    ...
    const handleOnSave = (id, newTitle, newBody) => {
      setPosts(
        posts.map((post) => {
          return (post.id === id ? {userId : post.userId, id : id, title : newTitle, body : newBody} : post)
        })
      );
    }

    const getButtonClass = (id) => {
      return allowedAuthorIds.includes(id) ? 'button-allowed' : 'button-not-allowed';
    }
  ...
  <div className='paging'>
      ...
      {currentPosts.map((post) => {
        return(
          <Post key = {post.id} post = {post} onSave = {handleOnSave}/>
